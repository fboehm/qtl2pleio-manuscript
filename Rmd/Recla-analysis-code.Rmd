...
Get the total number of markers (and pseudomarkers).

```{r, eval = FALSE}
sapply(FUN = length, X = pseudomap) -> lens
sum(lens)
```
...
The following code & text are taken from Karl's Rmd file where he analyzes Recla data:
...
```{r}
sex2 <- sex == "female"
scan_pvl(probs = pp, pheno = wlph[, c(10, 22)], covariates = sex2, kinship = kinship$`8`, start_snp1 = 1700, n_snp = 500) -> s1_out
calc_lrt_tib(s1_out)
```
...
```{r}
scan_pvl(probs = pp, pheno = wlph[, c(7, 22)], covariates = sex2, kinship = kinship$`8`, start_snp1 = 1700, n_snp = 500) -> s2_out
calc_lrt_tib(s2_out)
```
...
```{r}
scan_pvl(probs = pp, pheno = wlph[, c(7, 10)], covariates = sex2, kinship = kinship$`8`, start_snp1 = 1700, n_snp = 500) -> s3_out
calc_lrt_tib(s3_out)
```
